...
import { IProject, IProjectDoc, IProjectModel, IPackageDoc, IPackageModel } from './paginate.types';
...
projectSchema.virtual('packages', {
  ref: 'Package',
  localField: '_id',
...
describe('paginate plugin', () => {
  describe('populate option', () => {
    test('should populate the specified data fields', async () => {
      ...
      const newPackage = await Package.create({ name: 'Package One', project: project._id });

      const packagePages = await Package.paginate({ _id: newPackage._id }, { populate: 'project' });

      expect(packagePages.results[0]).toMatchObject({ project: { _id: project._id, name: project.name } });
    });
  });
